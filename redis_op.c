...
//    rop_test_reply_type(reply);
    ...
    max_count = (reply->elements > count) ? reply->elements : count;
    *get_num = max_count;
...
int rop_set_string(redisContext *conn, const char *key, char *value) 
{
    ...
    if (reply == NULL) 
    {
         LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]set: %s, value: %s Error:%s\n", key, value,conn->errstr);
         retn = -1;
         goto END;
    }
...
/* -------------------------------------------*/
/**
 * @brief  插入一个hash类型key field和value
 *
 * @param conn
 * @param key
 * @param field
 ...
 */
/* -------------------------------------------*/
int rop_set_hash(redisContext *conn, const char *key, char* field, const char *value) 
{
    int retn = 0;
    redisReply *reply = NULL;
    ...
    if (reply == NULL) 
    {
         ...
         LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]set: %s, field: %s, value: %s Error:%s\n", key, field, value,conn->errstr);
         retn = -1;
         goto END;
    }
 ...
 */
/* -------------------------------------------*/
int rop_get_hash(redisContext *conn, const char *key, char* field, char *value) 
{
    int retn = 0;
  redisReply *reply = NULL;
  ...
  if (reply == NULL)
  {
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]get: %s , field: %s Error:%s\n", key,field,conn->errstr);
        retn = -1;
        goto END;
  }
